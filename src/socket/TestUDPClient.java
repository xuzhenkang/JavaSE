...
/*
 * 注意ByteArrayOutputStream的所有构造函数都没有byte[] buf这样的形参，即定义ByteArrayOutputStream流对象时是不能指定byte数组的，
 * 因为这个连接到的byte数组是由ByteArrayOutputStream自动生成的 9行 API:"public ByteArrayOutputStream(): 
 ...
 * 但是ByteArrayOutputStream流并没有提供可以直接把long类型数据直接写入ByteArrayOutputStream流所连接到的byte数组中的方法,
